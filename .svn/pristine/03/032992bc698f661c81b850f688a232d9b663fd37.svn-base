					...
					if(data.data.loanQueryEntry.followTag!=-1){
						if(data.data.loanQueryEntry.followTag!='41') {
								if (data.data.loanQueryEntry.isSuccess == '1') {
									location.href='/html/loan/plans2/?type=4'
								}
								else {
									fn&&fn()
								}
	...
	var Loan={		
		start:function(){
			var _this=this,
				selects={
					city:['上海市','杭州市','宁波市'],
					money:['10-20万','20-30万','30-40万','40-50万']
				},timer=1;
			...
			_this.renderCounter({
				type:'other',
				limit:[{name:'12个月',value:12},{name:'24个月',value:24},{name:'36个月',value:36},{name:'60个月',value:60}],
		...
		submit:function(){
			var api=constant().url+'/user/loanInfo/submit',
				...
				data={
					...
					propertyCompany : _this.getValue('tenement','input'),
					clientType:constant().clientType,
					sessionKey: constant().sessionKey,
					loanType:4
				};
