$(function() {
		...
		function initInfo(data) {
			if (page.isSuccess(data)) {
				...
				if (page.accountInfo.freeCount == 0) {
					if(page.accountInfo.birdCoin < 5){
						page.isFee = true;
						page.deductionCoin=0;
					}else{
						page.isFee = false;
						page.deductionCoin=5;
					}
				} else {
					page.isFee = false;
					page.deductionCoin=0;
	...
	page.getBankList = function() {
		return page.getData(
			'/company/bankList', {}, [initTemp, page.bindBankList]);
	...
	//提现跳转
	page.tixianmoney = function(){
		var url = '/html/account/bank2_confirm/';
		var money2 = $('#txtWithdraw').val();
		var birdCoin=page.deductionCoin;
		page.bindBankInfo.birdCoin = birdCoin;
		page.bindBankInfo.money = money2;
		page.post(url, page.bindBankInfo);
	}
	//判断提现金额是否超出可用余额
	...
	page.init = function() {
		...
		page.when(
			page.getBankList, [page.initState]);
	...
	$('.goCenter').on('click', function() {
		window.location.href = '/html/account/detail2/?control=detailCharge'
	});
	//网银充值弹窗去帮助中心
