...
var page=new Page();
  ...
  page.operateContract=function(){
 	  return page.getData(
			'/invest/proxy/contract', param, [opeation]);
		...
		function opeation(data) {
			if (page.isSuccess(data)) {
        console.log(data)
				var url = data.data.redisKey;
				if(strUtil().isEmpty(url)){
          $('#confirm-cover,#modal-confirm').show();
          $('#modal-confirm .close-btn,#modal-confirm .btn').on('click',function() {
            window.close();
          })
				}else{
          console.log(constant().url+'/'+url)
					window.location.href=constant().url+'/'+url;
				}
			}
		};
  };
