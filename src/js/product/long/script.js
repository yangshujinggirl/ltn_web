  ...
  var secondCategoryId;
	...
	page.getLCTXL = function(cb) {
		...
		function getProductID(data) {
			if (data.resultCode == '0') {
				data = data.data;
        secondCategoryId = secondCategoryId = data.pcProductDetail.secondCategoryId
	...
	page.getLCTXQ = function(cb) {
			...
			function initTemp(data) {
				if (data.resultCode === '0') {
					data = data.data.pcProductIntroduceD;
          data.secondCategoryId = secondCategoryId;
					page.initTemplate(data, 'itemDetail', 'tempItemDetail');
				}
			}
	...
	page.operateEndMoney = function() {
		...
		if (endMoney <= startMoney) {
			...
			page.countBirdCoin(endMoney)
			page.countPayMoney(endMoney)
		}
	},
	//处理用户输入的金额
	...
	page.countBirdCoin = function(money) {
		...
		var moneyBirdCoin = parseInt((money / 50) * 100) / 100; //可用抵扣鸟币最大限制
		//判断鸟币余额是否大于最大限制
